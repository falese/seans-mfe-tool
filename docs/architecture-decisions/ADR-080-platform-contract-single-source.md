...
- **Committed example MFEs are untouched.** The template arrays lived inside
  `<% %>` blocks, so generated output is byte-identical; nothing needed
  regenerating.
...
**Worse.** `@seans-mfe/contracts` grows a module that is closer to
platform-specific knowledge than the envelope and error types it started as —
the package is drifting from "shared primitives" toward "shared contract",
and that boundary now needs watching. Consumers also gain a real coupling:
changing `PLATFORM_CAPABILITY_SPECS` changes DSL validation, codegen output,
and runtime guards at once, which is the point but makes the blast radius of
an edit larger than it was when each site was independently wrong.
