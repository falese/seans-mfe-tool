...
| **How a new framework is added** | Not possible without a different architectural approach; Angular MFE inside a React host requires separate composition machinery | Publish a new framework plugin package (`@falese/smt-framework-<name>`); implement the imperative `mount(element, props) → unmount` handle; no core platform change |
