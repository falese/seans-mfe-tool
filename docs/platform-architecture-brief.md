...
2. *Template-level context re-provision.* The ABC Kids example
   (`examples/abc-kids/`) now demonstrates the generic host-side composition
   path with React and Angular MFEs, a registry + daemon control plane, and
   provider-scoped layout slots. Generated templates still need to read
