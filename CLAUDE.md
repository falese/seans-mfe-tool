...
| `npm run check:mfe-drift:check` | After any `packages/codegen/**` change (templates included) — regenerates every `examples/**` MFE and fails on any drifted generator-owned file; the only check that compares generated bytes. CI-gated (`quality` job, #295) |
...
8. `npm run check:mfe-drift:check` (if you touched `packages/codegen/**` — templates included)
   ...
   has drifted. **This is the only gate that compares generated bytes**;
   `check:mfe-consistency` checks manifest ⇄ package.json ⇄ federation `shared` and
   passes happily on drifted files. Use the `:check` variant here — bare
...
10. `seans-mfe-tool compose:validate examples/<fleet> --check` (if you touched a
...
11. `npm run build:docs && git diff --exit-code docs/api`
