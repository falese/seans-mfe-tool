...
4. `npm run build`
5. `npm run build && git diff --exit-code schemas/` (if you changed command flags/types)
   ⚠️ `npm run build`, not `build:schemas` alone. The two halves of a command
   schema are read from different places: the **output** half comes from the
   TypeScript checker reading `src/`, and the **input** half comes from
   `Config.load()`, which reads `oclif.manifest.json` — generated by
   `npx oclif manifest` from the compiled `dist/`. So editing a flag's
   `description` and running only `build:schemas` regenerates the output half
   against source and the input half against a stale build, and the two halves
   of one file disagree. `npm run build` recompiles `dist/`, regenerates the
   manifest, then the schemas, in that order.
...
13. `npm run build:manifest-reference:check` (if you changed the manifest schema
    in `packages/dsl/src/schema.ts`) — `docs/schemas/manifest-fields.md` is a
    generated field reference, gated in the `quality` job. It is NOT covered by
    `build:schema:dsl:check`, which only proves the JSON Schema matches: the two
    artifacts are generated by different scripts from the same source, and the
    Markdown one was the half that went stale. `npm run build:manifest-reference`
    writes it.
14. `npm run check:swift-build` (if you touched `packages/framework-swift/**`)
    Compiles and tests the generated Swift package. Locally this **skips** unless
