...
| 6 | **framework-react**, **framework-angular**, **framework-swift**, **framework-rust** | Target plugins. Build lifecycle per target, plus (since ADR-097) its codegen contribution. The first two are `targetId: 'web'` — the primary build, chosen by the manifest's `framework`. **framework-swift** is `targetId: 'swift'`, chosen by `targets.swift`, and **framework-rust** is `targetId: 'rust'` (ADR-099); both run *alongside* the web plugin rather than instead of it (ADR-095). | `src/plugin.ts` |
...
- **framework plugins** (`framework-react`, `framework-angular`, `framework-swift`, `framework-rust`) add *build
