...
describe('One HTTP path, two error policies', () => {
  const withBff = () =>
    ({
      ...manifest(),
      targets: { swift: {} },
  ...
  it('builds the request in exactly one place', async () => {
    // doQuery and BFFClient each used to construct a URLRequest, set the same
    // headers and check the same 200..<300 range. `check:mfe-drift` compares
    // generated bytes to the template, so both stayed "correct" while free to
    // diverge, and no test touched either path.
    ...
    expect(base).toContain('public enum GraphQLPost');
    // The one construction site.
    expect(codeOnly(base).match(/URLRequest\(url:/g) ?? []).toHaveLength(1);
    expect(codeOnly(client)).not.toContain('URLRequest(url:');
    expect(codeOnly(client)).not.toContain('statusCode');
  });

  it('routes both callers through it', async () => {
    const files = await generate(withBff());
    const base = files.find((f) => f.path.endsWith('Platform/MFEBase.swift'))!.content;
    const client = files.find((f) => f.path.endsWith('Platform/BFFClient.swift'))!.content;
    expect(base).toContain('try await GraphQLPost.send(');
    expect(client).toContain('try await GraphQLPost.send(');
  });

  it('keeps the error policies different, which is the deliberate half', async () => {
    const files = await generate(withBff());
    const base = files.find((f) => f.path.endsWith('Platform/MFEBase.swift'))!.content;
    const client = files.find((f) => f.path.endsWith('Platform/BFFClient.swift'))!.content;
    ...
    expect(client).toContain('throw BFFError.graphQL(errors)');
  });

  it('makes the transport injectable, so query() is testable at all', async () => {
    // URLSession.shared was hardcoded in doQuery, so the capability could not
    // be exercised without real network. BFFClient already took a session.
    const base = (await fileNamed(withBff(), 'Platform/MFEBase.swift')).content;
    expect(base).toContain('public var transport: MFETransport?');
    expect(base).toContain('transport: deps.transport ?? GraphQLPost.urlSession');
    expect(codeOnly(base).match(/URLSession\.shared/g) ?? []).toHaveLength(1);
  });

  it('generates Swift tests that RUN both policies over a stub transport', async () => {
    const t = (await fileNamed(withBff(), 'Tests/MFETests/LifecycleTests.swift')).content;
    expect(t).toContain('queryReturnsPartialDataAndErrorsTogether');
    expect(t).toContain('bffClientThrowsOnTheSamePartialResponse');
    expect(t).toContain('queryForwardsTheBearerTokenAndHeaders');
    // Same bytes into both paths — that is what pins the difference.
    expect(t).toContain('partialResponse');
  });

  it('omits the BFFClient case when there is no BFF, keeping the rest', async () => {
    const bare = ({ ...manifest(), targets: { swift: {} } }) as unknown as DSLManifest;
    const t = (await fileNamed(bare, 'Tests/MFETests/LifecycleTests.swift')).content;
    expect(t).not.toContain('bffClientThrowsOnTheSamePartialResponse');
    expect(t).toContain('queryReturnsPartialDataAndErrorsTogether');
    expect(t).toContain('actor SeenRequest');
  });
});
