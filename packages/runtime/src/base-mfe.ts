...
export abstract class BaseMFE {
  ...
  constructor(manifest: DSLManifest, deps: BaseMFEDependencies = {}) {
    ...
    this.deps = deps;
    logger.debug('manifest keys', { data: Object.keys(manifest) });
    if (manifest.capabilities) {
      logger.debug('manifest capabilities', { data: JSON.stringify(manifest.capabilities) });
    }
  }
  ...
  protected async executeLifecycle(
    ...
    if (this._lifecycleStack.some(e => e.capability === capability && e.phase === phase)) {
      logger.error(`[BaseMFE] Re-entrant lifecycle detected for capability=${capability}, phase=${phase}. Aborting to prevent infinite loop.`);
      return;
    }
    this._lifecycleStack.push({capability, phase});
    logger.debug(`[BaseMFE] executeLifecycle: capability=${capability}, phase=${phase}`);
    ...
    if (!capabilityConfig?.lifecycle) {
      logger.debug(`[BaseMFE] No lifecycle hooks for capability=${capability}`);
    ...
    if (!hooks || hooks.length === 0) {
      logger.debug(`[BaseMFE] No hooks for capability=${capability}, phase=${phase}`);
      this._lifecycleStack.pop();
    ...
    for (const hookEntry of hooks) {
      logger.debug(`[BaseMFE] Executing hookEntry for capability=${capability}, phase=${phase}`);
  ...
  protected async invokeHandler(handlerName: string, context: Context): Promise<void> {
    logger.debug(`[BaseMFE] invokeHandler called for handlerName=${handlerName}`);
    if (handlerName.startsWith('platform.')) {
      ...
      if (this.deps?.platformHandlers && this.deps.platformHandlers[platformHandlerName]) {
        logger.debug(`[BaseMFE] Found DI platform handler: ${platformHandlerName}`);
        await this.deps.platformHandlers[platformHandlerName](context);
      } else {
        logger.debug(`[BaseMFE] Invoking platform handler: ${platformHandlerName}`);
      ...
      if (this.deps?.customHandlers && this.deps.customHandlers[customHandlerName]) {
        logger.debug(`[BaseMFE] Found DI custom handler: ${customHandlerName}`);
      ...
      if (lastSegment && this.deps?.customHandlers && this.deps.customHandlers[lastSegment]) {
        logger.debug(`[BaseMFE] Found DI custom handler (fallback): ${lastSegment}`);
        await this.deps.customHandlers[lastSegment](context);
        return;
      }
      // Fallback: invoke as method
      logger.debug(`[BaseMFE] Invoking custom handler: ${lastSegment}`);
  ...
  * ---
  ...
  protected async invokePlatformHandler(name: string, context: Context): Promise<void> {
    logger.debug(`[BaseMFE] invokePlatformHandler called for name=${name}`);
    ...
    logger.debug(`[BaseMFE] Executing platform handler function for ${name}`);
  ...
  protected async invokeCustomHandler(name: string, context: Context): Promise<void> {
    logger.debug(`[BaseMFE] invokeCustomHandler called for name=${name}`);
    const method = (this as any)[name];
    if (typeof method !== 'function') {
      logger.error(`[BaseMFE] Custom handler not found: ${name}`);
      throw new Error(
    ...
    logger.debug(`[BaseMFE] Executing custom handler method for ${name}`);
