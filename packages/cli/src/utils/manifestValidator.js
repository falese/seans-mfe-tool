...
function validateManifest(manifest) {
    ...
    if (manifest.performance) {
        errors.push('Legacy "performance" section detected. Please migrate to "plugins" and "transforms" sections (see ADR-062)');
    }
    ...
    if (manifest.plugins && Array.isArray(manifest.plugins)) {
        manifest.plugins.forEach((plugin, index) => {
            ...
            if (KNOWN_TRANSFORMS.includes(pluginName)) {
                errors.push(`Plugin at index ${index} "${pluginName}" is a transform, not a plugin. Move it to the "transforms" section.`);
            }
            else if (!KNOWN_PLUGINS.includes(pluginName)) {
                warnings.push(`Unknown plugin "${pluginName}" at index ${index}. Ensure it's a valid @graphql-mesh/plugin-* package.`);
            }
        });
    }
    ...
    if (manifest.transforms && Array.isArray(manifest.transforms)) {
        manifest.transforms.forEach((transform, index) => {
            ...
            if (KNOWN_PLUGINS.includes(transformName)) {
                errors.push(`Transform at index ${index} "${transformName}" is a plugin, not a transform. Move it to the "plugins" section.`);
            }
            else if (!KNOWN_TRANSFORMS.includes(transformName)) {
                warnings.push(`Unknown transform "${transformName}" at index ${index}. Ensure it's a valid @graphql-mesh/transform-* package.`);
            }
        });
    }
        ...
        if (typeof manifest.resolversComposition !== 'object') {
            ...
            Object.keys(manifest.resolversComposition).forEach((key) => {
                ...
                if (typeof value !== 'string') {
                    errors.push(`resolversComposition["${key}"] must be a string path to a composer function`);
                }
            });
        }
    }
