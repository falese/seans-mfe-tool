...
exports.KNOWN_MESH_TRANSFORMS = exports.KNOWN_MESH_PLUGINS = exports.DEFAULT_MESH_TRANSFORMS = exports.DEFAULT_MESH_PLUGINS = exports.DEPENDENCY_VERSIONS = void 0;
exports.validateManifestPlugins = validateManifestPlugins;
exports.validateManifestTransforms = validateManifestTransforms;
exports.validateManifestConfiguration = validateManifestConfiguration;
 ...
 */
exports.DEFAULT_MESH_TRANSFORMS = {
 ...
 */
exports.KNOWN_MESH_PLUGINS = new Set([
    'responseCache',
    'prometheus',
    'opentelemetry',
    'newrelic',
 ...
 */
exports.KNOWN_MESH_TRANSFORMS = new Set([
    'namingConvention',
    'rateLimit',
    'filterSchema',
 ...
 */
function validateManifestPlugins(manifest) {
    const result = {
        valid: true,
        errors: [],
        warnings: [],
    ...
    for (const pluginName of pluginEntries) {
        if (exports.KNOWN_MESH_PLUGINS.has(pluginName)) {
            result.classification.plugins.push(pluginName);
        }
        else if (exports.KNOWN_MESH_TRANSFORMS.has(pluginName)) {
            // This is a transform, not a plugin!
            result.errors.push(`"${pluginName}" is a transform, not a plugin. Move it to the "transforms" section.`);
            result.classification.transforms.push(pluginName);
            result.valid = false;
        }
        else {
            result.warnings.push(`Unknown plugin "${pluginName}". Ensure it's a valid @graphql-mesh/plugin-* package.`);
 ...
 */
function validateManifestTransforms(manifest) {
    const result = {
        valid: true,
        errors: [],
        warnings: [],
    ...
    const manifestTransforms = manifest.transforms;
    ...
    for (const transformName of transformEntries) {
        if (exports.KNOWN_MESH_TRANSFORMS.has(transformName)) {
            result.classification.transforms.push(transformName);
        }
        else if (exports.KNOWN_MESH_PLUGINS.has(transformName)) {
            // This is a plugin, not a transform!
            result.errors.push(`"${transformName}" is a plugin, not a transform. Move it to the "plugins" section.`);
            result.classification.plugins.push(transformName);
            result.valid = false;
        }
        else {
            result.warnings.push(`Unknown transform "${transformName}". Ensure it's a valid @graphql-mesh/transform-* package.`);
...
function validateManifestConfiguration(manifest) {
    ...
    if (allErrors.length > 0) {
        ...
        throw new Error(`Manifest validation failed with ${allErrors.length} error(s). ` +
    ...
    console.log(`✅ Manifest validation passed: ${totalPlugins} plugin(s), ${totalTransforms} transform(s)`);
}
// =============================
// Shared Utilities
// =============================
 ...
 */
function extractManifestVars(manifest) {
    ...
    const muiVersion = manifest.dependencies?.['design-system']?.['@mui/material'] || exports.DEPENDENCY_VERSIONS.mui.material;
    ...
    const neededTransforms = new Set();
    ...
    return {
        ...
        dependencyVersions: exports.DEPENDENCY_VERSIONS,
        ...
        meshPlugins: {
            responseCache: performanceConfig.caching?.enabled !== false ? exports.DEFAULT_MESH_PLUGINS.responseCache : null,
            prometheus: observabilityConfig.prometheus?.enabled !== false ? {
                ...exports.DEFAULT_MESH_PLUGINS.prometheus,
                ...observabilityConfig.prometheus,
            } : null,
            opentelemetry: observabilityConfig.opentelemetry?.enabled ? {
                ...exports.DEFAULT_MESH_PLUGINS.opentelemetry,
                ...observabilityConfig.opentelemetry,
            } : null,
        },
        meshTransforms: {
            namingConvention: exports.DEFAULT_MESH_TRANSFORMS.namingConvention,
...
async function renderTemplate(templatePath, vars) {
    ...
    return ejs_1.default.render(template, vars);
}
/**
...
async function writeGeneratedFiles(files, options = {}) {
    ...
    for (const file of files) {
        try {
            ...
            result.errors.push(`Failed to write ${file.path}: ${error.message}`);
        }
    }
    return result;
}
 ...
 */
async function generateAllFiles(manifest, basePath, options = {}) {
    // === Validation Layer (ADR-062) ===
    ...
    for (const entry of manifest.capabilities) {
        for (const [method, config] of Object.entries(entry)) {
            ...
            if (safeConfig.inputs)
                inputs = inputs.concat(safeConfig.inputs);
            if (safeConfig.outputs)
                outputs = outputs.concat(safeConfig.outputs);
        }
    }
    ...
    if (manifest.data) {
        ...
        const validSources = (manifest.data.sources || []).filter(source => source &&
            typeof source === 'object' &&
            source.name &&
            source.name.trim() &&
            source.handler);
        const meshBaseConfig = {
    ...
    if (await fs.pathExists(indexTemplatePath)) {
        ...
        const capabilityMetadata = domainCapabilities.map(name => {
            ...
            return {
                className: name,
                displayName: capConfig?.displayName || name,
                icon: capConfig?.icon || '📦'
            };
        });
