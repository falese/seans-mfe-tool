    ...
    static generateMethods(resourceName, modelName, pathGroup, dbAdapter) {
        ...
        for (const { path, operations, pathParameters } of pathGroup.paths) {
            for (const [method, operation] of Object.entries(operations)) {
                ...
                const implementation = ImplementationGenerator.generate(method, path, operation, modelName, dbAdapter);
                const methodContent = MethodGenerator.generateControllerMethod(functionName, method, path, operation, validations, implementation);
