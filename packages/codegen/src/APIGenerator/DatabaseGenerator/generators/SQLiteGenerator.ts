...
module.exports = ${pascalModelName};`;
    ...
    generateSchemaObject(schema) {
        ...
        return JSON.stringify(schemaObj, null, 2)
            .replace(/"DataTypes\.[^"]+"/g, (m) => m.slice(1, -1));
    }
    convertToSequelizeType(propName, property, required = false) {
        ...
        if (property.type === 'string') {
            if (property.minLength)
                validate.len = [property.minLength, property.maxLength || 255];
            if (property.pattern)
                validate.is = new RegExp(property.pattern);
        ...
        if (property.type === 'number' || property.type === 'integer') {
            if (property.minimum !== undefined)
                validate.min = property.minimum;
            if (property.maximum !== undefined)
                validate.max = property.maximum;
        }
