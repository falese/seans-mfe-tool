...
module.exports = {
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      ...
      const tables = [
        ${schemas.map(([schemaName]) => `'${NameGenerator.toModelName(schemaName)}s'`).reverse().join(',\n        ')}
      ];
    ...
    getColumnDefinition(propName, property, required = false) {
        ...
        return JSON.stringify(definition, null, 2)
            .replace(/"Sequelize\.[^"]+"/g, (m) => m.slice(1, -1))
            .split('\n')
            .join('\n          ');
    }
    ...
    generateForeignKeys(tableName, schema) {
        ...
        for (const [prop, config] of Object.entries(schema.properties)) {
            if (config['x-ref']) {
                ...
                foreignKeys.push(`await queryInterface.addConstraint('${tableName}', {
            fields: ['${columnName}'],
            type: 'foreign key',
