...
export async function bffValidateCommand(options: BFFCommandOptions = {}): Promise<BffValidateResult> {
  ...
  try {
    ...
    for (const source of meshConfig.sources) {
      if (!source.name) {
        throw new ValidationError('Each source must have a "name" property', 'source.name', 'required');
      }
      ...
      if (!specPath.startsWith('http')) {
        ...
        if (!await fs.pathExists(absoluteSpecPath)) {
          issues.push({ severity: 'warning', message: `OpenAPI spec not found: ${specPath}`, path: specPath });
    ...
    if (meshConfig.transforms) {
      ...
      for (const transform of meshConfig.transforms) {
        ...
        if (!validTransforms.includes(transformType)) {
          issues.push({ severity: 'warning', message: `Unknown transform type: ${transformType}`, path: transformType });
    ...
    if (meshConfig.plugins) {
      ...
      for (const plugin of meshConfig.plugins) {
        ...
        if (!knownPlugins.includes(pluginName)) {
          issues.push({ severity: 'warning', message: `Unknown plugin: ${pluginName} (may require additional package)`, path: pluginName });
