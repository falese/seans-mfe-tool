...
describe('meridianconcourseMFE', () => {
  ...
  beforeEach(() => {
    manifest = { name: 'meridian-concourse', version: '1.0.0', remoteEntry: 'http://localhost:5006/remoteEntry.js' };
    mfe = new meridianconcourseMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(meridianconcourseMFE);
  });
  ...
  it('should call MarketDirectory() and return an object', async () => {
    const result = await mfe.MarketDirectory(context);
