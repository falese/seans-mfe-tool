...
pub fn render(element: &Element, props: &serde_json::Value) -> Result<(), JsValue> {
    ...
    title.set_text_content(Some("PayStatus"));
    card.append_child(&title)?;

    let description = document.create_element("p")?;
    description.set_text_content(Some("Compact pay-status card for the console status rail — held and scheduled payroll at a glance"));
