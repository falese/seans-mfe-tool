...
/**
 * abckidsmazerunnerMFE bootstrap
 *
 * Instantiates abckidsmazerunnerMFE and calls load() so the full doLoad lifecycle
...
const manifest = {
  ...
  "remoteEntry": "http://localhost:3014/remoteEntry.js",
  "discovery": "http://localhost:3014/.well-known/mfe-manifest.yaml",
  "capabilities": [
    {
      "PlayGame": {
        "type": "domain",
...
export const mfe = new abckidsmazerunnerMFE(manifest);
...
export const mfeReady: Promise<void> = mfe
  ...
  .then((result) => {
    console.log(
      '[abckidsmazerunnerMFE] bootstrap load() complete \u2014 status:',
      result.status,
      '\u2014 components:',
  ...
  .catch((err) => {
    console.error('[abckidsmazerunnerMFE] bootstrap load() failed:', err);
  });
