...
import { abckidsmazerunnerMFE } from './mfe';
import type { Context } from '@seans-mfe-tool/runtime';

describe('abckidsmazerunnerMFE', () => {
  let mfe: abckidsmazerunnerMFE;
  let manifest: any;
  let context: Context;

  beforeEach(() => {
    manifest = { name: 'abc-kids-maze-runner', version: '1.0.0', remoteEntry: 'http://localhost:3014/remoteEntry.js' };
    mfe = new abckidsmazerunnerMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidsmazerunnerMFE);
  });
