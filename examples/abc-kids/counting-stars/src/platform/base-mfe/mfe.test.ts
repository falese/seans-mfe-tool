...
describe('abckidscountingstarsMFE', () => {
  ...
  beforeEach(() => {
    manifest = { name: 'abc-kids-counting-stars', version: '1.0.0', remoteEntry: 'http://localhost:3005/remoteEntry.js' };
    mfe = new abckidscountingstarsMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidscountingstarsMFE);
  });
