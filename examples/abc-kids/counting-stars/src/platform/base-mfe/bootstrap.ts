...
/**
 * abckidscountingstarsMFE bootstrap
 *
 * Instantiates abckidscountingstarsMFE and calls load() so the full doLoad lifecycle
...
const manifest = {
  ...
  "discovery": "http://localhost:3005/.well-known/mfe-manifest.yaml",
  "capabilities": [
    {
      "PlayGame": {
        "type": "domain",
...
export const mfe = new abckidscountingstarsMFE(manifest);
...
export const mfeReady: Promise<void> = mfe
  ...
  .then((result) => {
    console.log(
      '[abckidscountingstarsMFE] bootstrap load() complete \u2014 status:',
      result.status,
      '\u2014 components:',
  ...
  .catch((err) => {
    console.error('[abckidscountingstarsMFE] bootstrap load() failed:', err);
  });
