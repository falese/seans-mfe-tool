...
/**
 * abckidsanimalsoundsMFE bootstrap
 *
 * Instantiates abckidsanimalsoundsMFE and calls load() so the full doLoad lifecycle
...
export const mfe = new abckidsanimalsoundsMFE(manifest);
...
export const mfeReady: Promise<void> = mfe
  ...
  .then((result) => {
    console.log(
      '[abckidsanimalsoundsMFE] bootstrap load() complete \u2014 status:',
      result.status,
      '\u2014 components:',
  ...
  .catch((err) => {
    console.error('[abckidsanimalsoundsMFE] bootstrap load() failed:', err);
  });
