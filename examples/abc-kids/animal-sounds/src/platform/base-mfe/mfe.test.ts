...
describe('abckidsanimalsoundsMFE', () => {
  ...
  beforeEach(() => {
    manifest = { name: 'abc-kids-animal-sounds', version: '1.0.0', remoteEntry: 'http://localhost:3009/remoteEntry.js' };
    mfe = new abckidsanimalsoundsMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidsanimalsoundsMFE);
  });
