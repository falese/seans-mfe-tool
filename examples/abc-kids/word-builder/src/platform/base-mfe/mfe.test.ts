...
describe('abckidswordbuilderMFE', () => {
  ...
  beforeEach(() => {
    manifest = { name: 'abc-kids-word-builder', version: '1.0.0', remoteEntry: 'http://localhost:3012/remoteEntry.js' };
    mfe = new abckidswordbuilderMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidswordbuilderMFE);
  });
