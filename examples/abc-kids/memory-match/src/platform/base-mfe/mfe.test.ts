...
describe('abckidsmemorymatchMFE', () => {
  let mfe: abckidsmemorymatchMFE;
  let manifest: any;
  let context: Context;

  beforeEach(() => {
    manifest = { name: 'abc-kids-memory-match', version: '1.0.0', remoteEntry: 'http://localhost:3010/remoteEntry.js' };
    mfe = new abckidsmemorymatchMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidsmemorymatchMFE);
  });
