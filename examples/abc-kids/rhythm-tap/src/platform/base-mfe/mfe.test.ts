...
describe('abckidsrhythmtapMFE', () => {
  let mfe: abckidsrhythmtapMFE;
  let manifest: any;
  let context: Context;

  beforeEach(() => {
    manifest = { name: 'abc-kids-rhythm-tap', version: '1.0.0', remoteEntry: 'http://localhost:3013/remoteEntry.js' };
    mfe = new abckidsrhythmtapMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidsrhythmtapMFE);
  });
