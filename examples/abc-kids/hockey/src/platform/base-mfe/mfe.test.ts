...
describe('abckidshockeyMFE', () => {
  let mfe: abckidshockeyMFE;
  let manifest: any;
  let context: Context;

  beforeEach(() => {
    manifest = { name: 'abc-kids-hockey', version: '1.0.0', remoteEntry: 'http://localhost:3002/remoteEntry.js' };
    mfe = new abckidshockeyMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidshockeyMFE);
  });
