...
/**
 * abckidsrocketmathMFE bootstrap
 *
 * Instantiates abckidsrocketmathMFE and calls load() so the full doLoad lifecycle
...
const manifest = {
  ...
  "discovery": "http://localhost:3011/.well-known/mfe-manifest.yaml",
  "capabilities": [
    {
      "PlayGame": {
        "type": "domain",
...
export const mfe = new abckidsrocketmathMFE(manifest);
...
export const mfeReady: Promise<void> = mfe
  ...
  .then((result) => {
    console.log(
      '[abckidsrocketmathMFE] bootstrap load() complete \u2014 status:',
      result.status,
      '\u2014 components:',
  ...
  .catch((err) => {
    console.error('[abckidsrocketmathMFE] bootstrap load() failed:', err);
  });
