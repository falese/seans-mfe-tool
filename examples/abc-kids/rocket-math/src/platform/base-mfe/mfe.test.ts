...
import { abckidsrocketmathMFE } from './mfe';
import type { Context } from '@seans-mfe-tool/runtime';

describe('abckidsrocketmathMFE', () => {
  let mfe: abckidsrocketmathMFE;
  let manifest: any;
  let context: Context;

  beforeEach(() => {
    manifest = { name: 'abc-kids-rocket-math', version: '1.0.0', remoteEntry: 'http://localhost:3011/remoteEntry.js' };
    mfe = new abckidsrocketmathMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidsrocketmathMFE);
  });
