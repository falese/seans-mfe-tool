...
describe('abckidsflappyMFE', () => {
  let mfe: abckidsflappyMFE;
  let manifest: any;
  let context: Context;

  beforeEach(() => {
    manifest = { name: 'abc-kids-flappy', version: '1.0.0', remoteEntry: 'http://localhost:3001/remoteEntry.js' };
    mfe = new abckidsflappyMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidsflappyMFE);
  });
