...
import { abckidsshapesorterMFE } from './mfe';
import type { Context } from '@seans-mfe-tool/runtime';

describe('abckidsshapesorterMFE', () => {
  let mfe: abckidsshapesorterMFE;
  let manifest: any;
  let context: Context;

  beforeEach(() => {
    manifest = { name: 'abc-kids-shape-sorter', version: '1.0.0', remoteEntry: 'http://localhost:3007/remoteEntry.js' };
    mfe = new abckidsshapesorterMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidsshapesorterMFE);
  });
