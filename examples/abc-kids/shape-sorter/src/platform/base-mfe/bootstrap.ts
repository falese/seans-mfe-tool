...
/**
 * abckidsshapesorterMFE bootstrap
 *
 * Instantiates abckidsshapesorterMFE and calls load() so the full doLoad lifecycle
...
const manifest = {
  ...
  "discovery": "http://localhost:3007/.well-known/mfe-manifest.yaml",
  "capabilities": [
    {
      "PlayGame": {
        "type": "domain",
...
export const mfe = new abckidsshapesorterMFE(manifest);
...
export const mfeReady: Promise<void> = mfe
  ...
  .then((result) => {
    console.log(
      '[abckidsshapesorterMFE] bootstrap load() complete \u2014 status:',
      result.status,
      '\u2014 components:',
  ...
  .catch((err) => {
    console.error('[abckidsshapesorterMFE] bootstrap load() failed:', err);
  });
