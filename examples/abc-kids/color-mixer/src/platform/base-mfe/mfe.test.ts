...
describe('abckidscolormixerMFE', () => {
  let mfe: abckidscolormixerMFE;
  let manifest: any;
  let context: Context;

  beforeEach(() => {
    manifest = { name: 'abc-kids-color-mixer', version: '1.0.0', remoteEntry: 'http://localhost:3008/remoteEntry.js' };
    mfe = new abckidscolormixerMFE(manifest);
    context = { timestamp: new Date(), requestId: 'test-request-id' } as Context;
  });

  it('should instantiate without error', () => {
    expect(mfe).toBeInstanceOf(abckidscolormixerMFE);
  });
