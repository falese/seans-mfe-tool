...
const Model = require('../models');
  ...
  async function getAllPhaseMetrics(req, res, next) {
    ...
    try {
      ...
      const item = await Model.PhaseMetrics
      .find(req.query)
  ...
  async function getPhaseMetricsById(req, res, next) {
    ...
    try {
      ...
      const item = await Model.PhaseMetrics.findById(req.params.phaseId);
      if (!item) {
        throw new ApiError(404, 'Not found');
      }
      res.status(200).json(item);
  ...
  async function updatePhaseMetrics(req, res, next) {
    ...
    const body = req.body;
      ...
      const updatedItem = await Model.PhaseMetrics.findByIdAndUpdate(req.params.phaseId, req.body, { new: true });
      if (!updatedItem) {
        throw new ApiError(404, 'Not found');
      }
